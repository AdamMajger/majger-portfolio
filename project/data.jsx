...
// All visuals are generative striped SVG placeholders w/ monospace explainers,
// because we don't have real photography yet.
...
// Remap legacy project categories onto the new 4-category system.
...
// Variant controls stripe orientation / density / contrast for variety.
  ...
  return (
    <div className={`ph ph--${animate || "static"}`} style={{
      ...
      <svg width="100%" height="100%" viewBox="0 0 400 500" preserveAspectRatio="none"
        ...
        {/* A single accent shape per variant to differentiate */}
...
// Projects. Slovak event/visual design context — invented for the prototype.
const PROJECTS = [
  {
    slug: "garaz",
    title: "Garáž",
  ...
  },
  {
    ...
    ratio: "16/9",
    // Use the real YouTube thumbnail as the card visual.
...
const CV = [
  ...
  { period: "2015 — 2023",          role: "Juior Designer → Art Director",   place: "Triad Advertising" },
...
// Apply the remap so existing project data plugs into the new 4 filters.
PROJECTS.forEach((p) => {
  if (CATEGORY_REMAP[p.category]) p.category = CATEGORY_REMAP[p.category];
});

// Promote two projects to in-grid YouTube embeds as an example.
// `videoId` here is a real, embeddable, royalty-free clip so the demo plays
// even with no project-specific footage. Replace with the project's real ID.
const VIDEO_GRID = {
  "letne-kino-tehelne-pole": "WjJTNZQUVAo", // Big Buck Bunny — placeholder reel
  "biela-noc":               "aqz-KE-bpKQ", // Same clip family — second example
};
PROJECTS.forEach((p) => {
  if (VIDEO_GRID[p.slug]) p.videoId = VIDEO_GRID[p.slug];
});
